                        ...
                        <div class="card-body">
                          <div class="table-responsive">
                            <table class="table table-bordered">
                              ...
                              <tbody>
                                 <?php
                                    function convertPeriodeKeTanggal($periode) {
                                        $bulan = array_flip(['Januari', 'Februari', 'Maret', 'April', 'Mei',
                                                             'Juni', 'Juli','Agustus','September','Oktober',
                                                             'November','Desember']);
                                        $split = explode(' ', $periode);
                                        
                                        // kurangi satu dari bagian tahun
                                        $tahun = (int)$split[1] - 1;
                                        
                                        return sprintf('%04d-%02d-01',$tahun , $bulan[$split[0]] + 1);
                                    }
                                    $tanggal = convertPeriodeKeTanggal($periode);
                                    $no=1;
                                    $query = mysqli_query($koneksi, "
                                      SELECT e.*, b.*, r.nilai, p.kode_periode 
                                      FROM evaluasi_wp as e 
                                      INNER JOIN bahanbaku as b ON e.kode_bahan = b.kode_bahan 
                                      INNER JOIN periode as p ON p.tanggal = '$tanggal'
                                      INNER JOIN relasi as r ON r.kode_periode = p.kode_periode AND r.kode_bahan = e.kode_bahan
                        ...
                        function drawChart() {
                           if ($("#grafikprediksi").length > 0) {
                              var a = google.visualization.arrayToDataTable([
                                ...
                                <?php
                                    $query = mysqli_query($koneksi, "SELECT * FROM evaluasi_wp as e, bahanbaku as b WHERE e.kode_bahan=b.kode_bahan AND e.periode='$periode' ORDER BY e.c1 DESC");
                                    while($rows=mysqli_fetch_array($query)){
